...
async def test_search_finds_article_via_keyword_from_longer_client_message(tenant_id):
    async with tenant_connection(tenant_id) as conn:
        await conn.execute(
            """
            INSERT INTO knowledge_base_articles (tenant_id, kind, title, body, topic_tag)
            ...
            tenant_id,
            "known_issue",
            "Клики не фиксируются после обновления",
            "Известная проблема после обновления Keitaro 10.x.",
            "tracking_failure",
        )

    results = await knowledge_base.search(
        tenant_id,
        "После обновления кампании не фиксируют клики совсем, нет данных в статистике.",
    )

    assert len(results) == 1
    assert results[0].title == "Клики не фиксируются после обновления"
