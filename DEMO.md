...
Нужны: Python 3.12+, `.venv` с зависимостями (`pip install -e ".[dev]"`), `.env` с
...
## 3. Сценарии для показа (в терминале, реальные вызовы Anthropic API — каждый ~10-60 сек)
...
**Опционально — замкнуть Approval Gate живьём** (после сценария B, чтобы показать
"черновик -> человек -> публикация" целиком): взять `approval_id`, напечатанный в выводе
сценария B, и через MCP-инструменты `approval_gate_approve` -> `kb_publish_approved`
(`src/etiology/main.py:build_app()` собирает готовый MCP-сервер со всеми инструментами;
самый быстрый способ показать — короткий inline-скрипт, как в истории сессии, либо любой
MCP-клиент).

## 4. Если что-то не подключается
...
## 5. Известные, осознанные ограничения (говорить прямо, не прятать)
